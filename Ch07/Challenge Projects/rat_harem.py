...
def selection(population, to_retain):
    ...
    males_to_retain_by_sex = to_retain // 4  # floor division to split the number of rats 1:4 ration male:female
    females_to_retain_by_sex = to_retain - males_to_retain_by_sex
    males_count = len(sorted_population)//4  # we want to take the parents and divide 1:4 to count males and females
    female_count = len(sorted_population) - males_count 
    females = sorted_population[:female_count]  # since males are larger than females, assume the first half of the sorted list is females
    males = sorted_population[males_count:]  #  accordingly, the second half of the list should be males
    selected_females = females[-females_to_retain_by_sex:]  # use negative slicing to grab the back half of the list - the largest females
    selected_males = males[-males_to_retain_by_sex:]  # use negative slicing to grab the back half of the list - the largest males
