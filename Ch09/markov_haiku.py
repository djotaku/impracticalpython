...
def map_word_to_corpus(corpus):
    ...
    logging.debug(f'map_word_to_word results for \"sake\" = {dict1_to_1["sake"]} \n')
    return dict1_to_1
...
def map_2_words_to_word(corpus):
    ...
    logging.debug(f'map_2_words_to_word results for \"sake jug\" = {dict2_to_1["sake jug"]}\n')
    return dict2_to_1
...
def word_after_double(prefix, suffix_map_2, current_syls, target_syls):
    """Return all acceptable words in a corpus that follwo a word pair."""
    accepted_words = []
...
def haiku_line(suffix_map_1, suffix_map_2, corpus, end_prev_line, target_syls):
    ...
    while True:
        ...
        logging.debug(f"{line_syls + num_syls=}")
...
def main():
    ...
    while choice != "0":
        ...
        # some unknown choice
            ...
            print("\nSorry, but that isn't a valid chioce.", file=sys.stderr)
            continue
