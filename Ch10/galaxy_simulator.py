...
# scale (radio buggle diamter) in light-years
...
root = tk.Tk()
root.title("Milkey Way galaxy")
c = tk.Canvas(root, width=1000, heigh=800, bg='black')
c.grid()
c.configure(scrollregion=(-500, -400, 500, 400))

# actual Mikley Way dimensions (light-years)
DISC_RADIUS = 50000
DISC_HEIGHT = 1000
...
def random_polar_coordinates(disc_radius_scaled):
    """Generate uniform random (x, y) point within a disc for 3D display."""
    r = random()
...
def star_haze(disc_radius_scaled, density):
    ...
    disc_radius_scaled = galactic disc radius scaled to radio buble diameter
...
def main():
    """Calculate detection probablity & post galaxy display & statistics."""
